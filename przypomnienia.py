...
cursor.execute(
    "CREATE TABLE IF NOT EXISTS wydarzenia (id INTEGER PRIMARY KEY AUTOINCREMENT, nazwa TEXT, data DATE, opis TEXT, uid BIGINT)")
...
class przypomnienia(commands.Cog):
            ...
            if rows_w:
                ...
                for row_w in rows_w:
                    if now < datetime.datetime.strptime(row_w[2], '%d.%m.%Y %H:%M'):
                        if rows_p:
                            for row_p in rows_p:
                                if row_w[0] == row_p[0] and datetime.datetime.strftime(now, '%d.%m.%Y %H:%M') == row_p[
                                    1]:
                                    user = await self.bot.fetch_user(row_w[4])
                                    msg = 'Cześć, przypominam, że ' + row_w[
                                        2] + ' odbędzie się Twoje wydarzenie o nazwie ' + row_w[1] + '.'
                                    if row_w[3]:
                                        msg += '\nOpis wydarzenia: ' + row_w[3]
                                    await user.send(msg)
                                    cursor2.execute(
                                        "DELETE FROM przypomnienia WHERE id=\"" + str(row_w[0]) + "\" AND data=\"" +
                                        row_p[1] + "\"")
                                    connection.commit()
                        ...
                        user = await self.bot.fetch_user(row_w[4])
                        msg = 'Cześć, przepraszam, że nie mogłem przypomnieć Ci o Twoim wydarzeniu ' + row_w[
                            1] + ', które odbyło się ' + row_w[2] + ' :('
                        if row_w[3]:
                            msg += '\nOpis wydarzenia: ' + row_w[3]
                        await user.send(msg)
                        cursor2.execute("DELETE FROM przypomnienia WHERE id=\"" + str(row_w[0]) + "\"")
    ...
    @bot.command(name='event',
                 aliases=['wydarzenie', 'przypomnienie', 'notification', 'new', 'add', 'nowe', 'reminder'],
                 description='Pozwala na dodanie nowego wydarzenia: =event nazwa data godzina opis(między znakami -) daty przypomnień')
    async def reminder_command(self, ctx, nazwa, data, godzina, *czasy):
        ...
        for x in czasy:
            ...
            if x[0] == '-':
                ...
                try:
                    ...
                    if 'm' in x:
                        p = datetime.datetime.strftime(data_ - datetime.timedelta(minutes=int(x.split('m')[0])),
                                                       '%d.%m.%Y %H:%M')
                    elif 'h' in x:
                        p = datetime.datetime.strftime(data_ - datetime.timedelta(hours=int(x.split('h')[0])),
                                                       '%d.%m.%Y %H:%M')
                    elif 'd' in x:
                        p = datetime.datetime.strftime(data_ - datetime.timedelta(days=int(x.split('d')[0])),
                                                       '%d.%m.%Y %H:%M')
                    elif 'w' in x:
                        p = datetime.datetime.strftime(data_ - datetime.timedelta(weeks=int(x.split('w')[0])),
                                                       '%d.%m.%Y %H:%M')
                    else:
        ...
        if n:
            await ctx.send(
                'Możesz ustawić maksymalnie 5 przypomnień, dlatego ostatnie ' + str(n) + ' nie zostały dodane.')

        cursor.execute("INSERT INTO wydarzenia (nazwa,data,opis,uid) VALUES(\"" + nazwa + "\",\"" + data_.strftime(
            '%d.%m.%Y %H:%M') + "\",\"" + opis + "\",\"" + str(ctx.author.id) + "\")")
        for x in czasy_:
            cursor2.execute("INSERT INTO przypomnienia VALUES(\"" + str(cursor.lastrowid) + "\",\"" + x + "\")")
        connection.commit()
        msg = 'Przypomnienie wydarzenia ' + nazwa + ', które odbędzie się ' + data_.strftime(
            '%d.%m.%Y %H:%M') + ' zostanie przypomniane'
        if czasy_:
    ...
    @bot.command(name='events', aliases=['wydarzenia', 'wyświetl_wydarzenia', 'show_events'],
                 description='Wyświetla wydarzenia użytkownika w prywatnej wiadomości')
    async def events_command(self, ctx):
        ...
        if rows_w:
            msg = '```'
            for row_w in rows_w:
                msg += 'ID: ' + str(row_w[0]) + ' Wydarzenie ' + row_w[1] + ' odbędzie się ' + row_w[2] + '. Opis: ' + \
                       row_w[3] + '.\nPrzypomnienia: '
                for row_p in rows_p:
                    if row_w[0] == row_p[0]:
                        msg += row_p[1] + ' '
    ...
    @bot.command(name='delete', aliases=['usuń_wydarzenie', 'delete_event'],
                 description='Usuwa wydarzenie od podanym id')
    ...
    @bot.command(name='edit', aliases=['edytuj', 'edit_event', 'edytuj_wydarzenie'],
                 description='Pozwala na edycję wydarzenia o podanym id: =edit id typ (nazwa, data, opis, przypomnienie) dane')
                ...
                if int(row_w[0]) == int(id_):
                    ...
                    while i < len(edit):
                        if edit[i] == 'nazwa' or edit[i] == 'name':
                            if edit[i + 1]:
                                cursor.execute(
                                    "UPDATE wydarzenia SET nazwa = \"" + str(edit[i + 1]) + "\" WHERE id = \"" + str(
                                        id_) + "\"")
                                connection.commit()
                                i += 2
                            else:
                            ...
                            cursor.execute(
                                "UPDATE wydarzenia SET opis = \"" + opis + "\" WHERE id = \"" + str(id_) + "\"")
                            connection.commit()

                        elif edit[i] == 'data' or edit[i] == 'date':
                            if parse(edit[i + 1] + ' ' + edit[i + 2]) and parse(
                                    edit[i + 1] + ' ' + edit[i + 2]) > datetime.datetime.now():
                                i += 3
                                cursor.execute(
                                    "UPDATE wydarzenia SET data = \"" + parse(edit[i - 2] + ' ' + edit[i - 1]).strftime(
                                        '%d.%m.%Y %H:%M') + "\" WHERE id = \"" + str(id_) + "\"")
                                connection.commit()
                            else:
                            ...
                            if rodzaje.index(typ) < 2:
                                if not len(dates) % 2:
                                    ...
                                    while j < len(dates):
                                        if any(dates[j + 1].strftime('%d.%m.%Y %H:%M') in x for x in rows_p):
                                            await ctx.send(
                                                'Nie możesz wielokrotnie podać takiej samej daty przypomnienia!')
                                            return
                                        for x in range(len(rows_p)):
                                            if dates[j].strftime('%d.%m.%Y %H:%M') == rows_p[x][1]:
                                                if reminder_check(ctx, dates[j + 1],
                                                                  datetime.datetime.strptime(row_w[2],
                                                                                             '%d.%m.%Y %H:%M')):
                                                    return

                                                cursor2.execute(
                                                    "UPDATE przypomnienia SET data = \"" + dates[j + 1].strftime(
                                                        '%d.%m.%Y %H:%M') + "\" WHERE id = \"" + str(
                                                        id_) + "\" AND data = \"" + rows_p[x][1] + "\"")
                                                connection.commit()
                                                cursor2.execute(
                                                    "SELECT * FROM przypomnienia WHERE id = \"" + str(id_) + "\"")
                                                rows_p = cursor2.fetchall()
                                ...
                                for x in dates:
                                    ...
                                    for y in rows_p:
                                        ...
                                        if x.strftime('%d.%m.%Y %H:%M') == y[1]:
                                            cursor2.execute("DELETE FROM przypomnienia WHERE id = \"" + str(
                                                id_) + "\" AND data = \"" + y[1] + "\"")
                                            connection.commit()
                                            j = 0
                                            break
                                ...
                                for x in dates:
                                    if reminder_check(ctx, x, datetime.datetime.strptime(row_w[2], '%d.%m.%Y %H:%M')):
                                        return
                                    if any(x.strftime('%d.%m.%Y %H:%M') in y for y in rows_p) or len(dates) != len(
                                            set(dates)):
                                        await ctx.send('Nie możesz wielokrotnie podać takiej samej daty przypomnienia!')
                                        return
                                    cursor2.execute(
                                        "INSERT INTO przypomnienia VALUES(\"" + str(id_) + "\",\"" + x.strftime(
                                            '%d.%m.%Y %H:%M') + "\")")
                                    connection.commit()
